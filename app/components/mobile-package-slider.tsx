...
export default function MobilePackageSlider({ packages }: { packages: PackageItem[] }) {
  ...
  return (
    <div className="mt-12 lg:hidden">
      <div
        ref={viewportRef}
        className="package-slider__viewport -mx-6 flex snap-x snap-mandatory gap-4 overflow-x-auto px-6 pb-2"
      ...
      >
        {packages.map((item, packageIndex) => (
          <article
            key={item.name}
            data-package-slide
            className={`min-h-full w-[min(19.5rem,82vw)] shrink-0 snap-center rounded-lg border p-6 ${
              item.featured
